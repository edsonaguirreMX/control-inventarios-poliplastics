...
**Fecha:** 2026-08-10 · **Rama:** `pr-8-hardening-deploy` · **Suite al cierre:** 185/185 tests verdes (`npx vitest run`).
...
|---|---|---|---|
| `login` | action pública | Sin rol (es el propio login, ver excepción arriba) — rate limit atómico por usuario (5 intentos/15min, `admitirIntentoLogin` checa Y reserva en una sola mutation) antes de tocar bcrypt | `auth.test.ts` (10 tests, EDS-70 + bloqueante y mayores de esta ronda) |
...
| `admitirIntentoLogin`, `limpiarIntentosLogin`, `limpiarLoginIntentosExpirados`, `getUserByUsuario`, `createSession` | internas | Llamadas solo desde `authActions.login` (o desde el cron de limpieza), que ya autorizó/no requiere contexto de usuario. `limpiarIntentosLogin` decrementa en 1 (deshace solo la reserva del request exitoso) en vez de borrar toda la fila — no pisa reservas de otros intentos concurrentes en vuelo (hallazgo #8 de esta ronda). `limpiarLoginIntentosExpirados` se reprograma a sí misma si un lote de limpieza sale lleno, para no quedarse fija en 200 filas/hora bajo un flood de usuarios únicos (hallazgo #9) | `auth.test.ts` |
...
| 8 | **Re-review de CodeRabbit sobre el commit `ae8d49b`**: `limpiarIntentosLogin` borraba la fila COMPLETA de `loginIntentos` al primer login exitoso. Bajo intentos concurrentes, eso también borraba las reservas de OTROS intentos del mismo usuario que seguían en vuelo (ya admitidos por `admitirIntentoLogin`, contados en `intentos`, con su `bcrypt` todavía sin resolver) — un login legítimo concurrente le devolvía al atacante un cupo nuevo de 5 intentos gratis | Mayor | `limpiarIntentosLogin` ahora decrementa `intentos` en 1 (deshace solo la reserva propia) en vez de borrar toda la fila; solo borra la fila si el conteo llega a 0. Efecto secundario intencional y documentado: un login exitoso ya no perdona TODOS los fallos previos, solo su propia reserva — 2 tests nuevos en `auth.test.ts` (reservas ajenas preservadas + caso común de fila sola) y el test preexistente de "limpia el contador" actualizado a la nueva semántica |
| 9 | **Re-review de CodeRabbit sobre el commit `ae8d49b`**: la limpieza de `loginIntentos` estaba acotada a 200 filas por corrida de cron (cada hora) — un atacante que manda >200 usuarios distintos por hora crea filas expiradas más rápido de lo que esa limpieza las borra; el rezago podía crecer sin límite igual | Mayor | `limpiarLoginIntentosExpirados` se reprograma a sí misma (`ctx.scheduler.runAfter(0, ...)`) cuando un lote sale lleno, en vez de esperar la próxima hora — le sigue el paso a un flood mientras dure. No sustituye throttling por IP (la app no tiene esa señal por diseño, es por `usuario`); test nuevo con 250 filas expiradas simuladas confirma que la continuación vacía el rezago completo en la misma corrida lógica |
...
**EDS-66 (10.2): completo, tras dos rondas de No-Go/re-review sobre la propia auditoría (PR #9, 2026-08-10).**

- **Ronda 1 (No-Go del usuario):** encontró un bloqueante real (carrera en el rate limit, no cubierta por los tests secuenciales originales) y 3 mayores (limpieza de `loginIntentos`, y dos correcciones a este mismo documento) — los 4 corregidos en el commit `ae8d49b`, junto con 3 menores de estilo.
- **Ronda 2 (re-review de CodeRabbit sobre `ae8d49b`, a petición explícita del usuario):** encontró 2 mayores más, ambos genuinos y no hipotéticos: `limpiarIntentosLogin` seguía borrando la fila completa en vez de solo la reserva propia (hallazgo #8 — el mismo punto que CodeRabbit ya había pedido en la ronda 1 y que su propio auto-resuelto había marcado "atendido" sin serlo del todo), y la limpieza de `loginIntentos` no le seguía el paso a un flood de >200 usuarios únicos por hora (hallazgo #9). Ambos corregidos en el commit siguiente, con verificación en vivo del rate limit (5 fallos + 1 intento con password correcto, ambos contra el deployment real, no solo `convex-test`) además de los tests automatizados.

Autorización server-side confirmada módulo por módulo con evidencia citable, incluidas sus excepciones intencionales explícitas; integridad de inventario confirmada contra los 3 criterios del plan; 12 tests nuevos en total entre la auditoría original y las dos rondas de fixes, cerrando gaps de cobertura y de concurrencia reales; hardening de XSS de PR8 documentado con su verificación manual. Suite completa: **185/185** (`npx vitest run`).
