...
- **Sesiones**: token aleatorio, dos duraciones según si el usuario marca "recordar en este dispositivo" — 30 días (persistente, `localStorage`) vs. ~12 horas (se pierde al cerrar pestaña, `sessionStorage`).
  - **Riesgo aceptado, no un patrón a copiar sin más**: guardar el token en Web Storage (en vez de una cookie `HttpOnly`) lo expone a lectura por cualquier JavaScript que corra en el mismo origen — si alguna vez apareciera una vulnerabilidad de XSS en el sistema, ese token quedaría robable. Se aceptó conscientemente en este proyecto (sistema interno, sin contenido de terceros embebido, superficie de XSS reducida por el escape sistemático de contenido dinámico, ver sección 20), **no** porque sea la opción más segura en abstracto — un sistema con más superficie de ataque debería preferir una cookie `Secure`+`HttpOnly`+`SameSite` en su lugar.
  - **Gap conocido en el logout**: el cliente borra el token local **antes** de esperar la confirmación del servidor (para que la sesión se sienta cerrada de inmediato en la pantalla); si esa llamada al servidor falla por red o se agota su timeout corto, la fila de sesión **sigue viva en el servidor** aunque el dispositivo ya no tenga el token a mano — una ventana real, aunque acotada, donde el token (si alguien más lo hubiera copiado) seguiría siendo válido. Documentado y aceptado como riesgo conocido en este proyecto, no resuelto con revocación server-side forzada.
...
- **Logout automático por inactividad**: 10 minutos sin interacción (mouse/teclado/scroll/touch/click) cierran la sesión del lado del cliente, revisando cada 15 segundos (no en cada evento, para no sobrecargar). Mismo mecanismo de logout (y mismo gap) que el logout manual, arriba.
